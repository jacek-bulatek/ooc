...
import com.packtpub.libgdx.orderofchaos.components.ComponentSubject;
...
public abstract class PhysicsComponent extends ComponentSubject implements Component {
    ...
    protected AbilityHitbox _ability1Hitbox;
    ...
    protected boolean isCollisionWithHitbox(Entity sourceEntity, AbilityHitbox hitbox, MapManager mapMgr){
    ...
    public Array<Drawable> getDebugDrawables(){
        Array<Drawable> drawables = new Array<>();
        if(_boundingBox != null) {
            drawables.add(_boundingBox);
        }
        if(_ability1Hitbox != null) {
            drawables.add(_ability1Hitbox);
        }
        return drawables;
    }
}
